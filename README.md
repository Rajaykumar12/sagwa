...
| Reference-based and safety metrics | Exact/fuzzy match, ROUGE-L, embedding similarity (where ground truth exists); PII-regex and toxicity-keyword flags per case | Built and tested |
...
| `sagwa diff` | Per-metric/per-tag regression detection with statistical-significance testing | Not implemented, CLI stub that prints a message and exits 1 |
| Failure clustering | Embed failing cases, HDBSCAN cluster, auto-label | Not started, `sagwa/clustering/` is empty |
| `sagwa gate` and CI Action | Threshold-based CI gating from `config/gates.yaml` | Not implemented, CLI stub; the CI workflow is a documented placeholder |
...
| Layer | Choice |
|---|---|
| CLI | Typer (`sagwa run`, `sagwa diff`, `sagwa gate`), installed as a console script |
...
| CI | GitHub Actions, currently a placeholder workflow |
...
That result is persisted as `Run` and `Result` rows (`sagwa/storage/models.py`) via `get_session()`.
...
- `sagwa/cli.py`: `sagwa run|diff|gate` entry points plus adapter loading
- `sagwa/adapters/`: `TargetAdapter` protocol and `AdapterResult`; stub adapter only
...
- `sagwa/metrics/`: `reference.py`, `safety.py` (live); `ragas_metrics.py` (blocked)
- `sagwa/judge/`: `harness.py` (absolute/pairwise scoring); `calibration.py`
- `sagwa/diff/`: empty, regression/statistical-significance diff engine, roadmap
- `sagwa/clustering/`: empty, HDBSCAN failure clustering, roadmap
- `sagwa/dashboard/`: empty, Streamlit dashboard, roadmap
...
|---|---|---|
| `GROQ_API_KEY` | LLM-as-judge (`sagwa/judge/`) | The only external API this project calls directly. |
...
See `examples/adapters/README.md` for the full walkthrough.
...
Both commands exist in the CLI today as explicit placeholders so the intended interface is visible ahead of the implementation. Gate thresholds are already defined in version-controlled config (`config/gates.yaml`), for example:
...
The CI workflow documents the intended shape but currently just echoes a TODO and exits `0`.

## Roadmap and Contributing

### Current status

Golden-set schema/loader, run-history storage, adapter contract, the async runner, reference/safety metrics, and the judge and calibration engine are all built and tested. The RAGAS integration and the ringo adapter are implemented but blocked/unvalidated (see the feature table above). The real ~150-200-case human calibration study, the project's credibility anchor, has not been run yet.

### Upcoming

- Diff engine with statistical-significance testing; HDBSCAN failure clustering with auto-labeling
- CI gate (GitHub Action and CLI), tracing integration, cost/latency tracking
- Streamlit dashboard (trend lines, cluster browser, run diff view)
...
1. Read a module's own docstring before touching an unimplemented one (`runner/`, `metrics/`, `judge/`, `diff/`, `clustering/`, `dashboard/`), each was scaffolded deliberately, not left empty by accident.
